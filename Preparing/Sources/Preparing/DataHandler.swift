...
struct DataHandler {
        static func generateNotations() -> [Notation] {
                ...
                let entries: [Notation?] = sourceLines.dropFirst().map { line -> Notation? in
                        ...
                        let parts: [String] = blocks.count >= 18 ? blocks : (blocks + fill)
                        ...
                        let literaryColloquial: Int = Int(parts[keys.literaryColloquial]) ?? 0
                        let frequency: Int = Int(parts[keys.frequency]) ?? 0
                        ...
                        let colloquial: String = parts[keys.colloquial]
                        ...
                        let entry: Notation = Notation(word: word, jyutping: jyutping, shortcut: shortcut, ping: ping, pronunciationOrder: pronunciationOrder, isSandhi: isSandhi, literaryColloquial: literaryColloquial, frequency: frequency, altFrequency: altFrequency, partOfSpeech: partOfSpeech, register: register, label: label, written: written, colloquial: colloquial, english: english, urdu: urdu, nepali: nepali, hindi: hindi, indonesian: indonesian)

                        return entry
                }
                return entries.compactMap({ $0 }).uniqued()
        }
